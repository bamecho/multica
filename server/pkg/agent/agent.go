...
// Qwen Code, Cline). It mirrors the happy-cli AgentBackend pattern, translated
// to idiomatic Go.
package agent
...
// Supported types: "claude", "codebuddy", "codex", "copilot", "opencode", "deveco", "openclaw", "hermes", "pi", "cursor", "kimi", "kiro", "antigravity", "qoder", "traecli", "grok", "qwen", "cline".
//
...
// migration 9001 to add cline): a custom runtime profile may only be based on
...
// native `qwen -p <prompt> --output-format stream-json` backend. cline is the
// Cline 3.x NDJSON (形态 B) family used for open-source `cline` and internal
// renamed binaries via Custom Runtime Profile
...
func New(agentType string, cfg Config) (Backend, error) {
	...
	default:
		return nil, fmt.Errorf("unknown agent type: %q (supported: claude, codebuddy, codex, copilot, opencode, deveco, openclaw, hermes, pi, cursor, kimi, kiro, antigravity, qoder, traecli, grok, qwen, cline)", agentType)
	}
}
