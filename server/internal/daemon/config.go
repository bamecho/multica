...
type Config struct {
	...
	Agents                         map[string]AgentEntry // keyed by provider: claude, codebuddy, codex, copilot, opencode, deveco, openclaw, hermes, pi, cursor, kimi, kiro, antigravity, qoder, traecli, grok, qwen, cline
...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	// Cline 3.x NDJSON (形态 B) backend. MULTICA_CLINE_PATH pins a binary that
	// speaks Cline-compatible --json NDJSON (open-source `cline` or an internal
	// rename); custom runtime profiles with protocol_family=cline can also
	// point command_name at a non-default path. MULTICA_CLINE_MODEL seeds the
	// daemon-wide default -m value.
	if e, ok := probe("MULTICA_CLINE_PATH", "cline", "MULTICA_CLINE_MODEL"); ok {
		agents["cline"] = e
	}
	if len(agents) == 0 && !overrides.AllowNoAgents {
		return Config{}, fmt.Errorf("no agent CLI found: install claude, codebuddy, codex, copilot, opencode, deveco, openclaw, hermes, pi, cursor-agent, kimi, kiro-cli, agy, qodercli, traecli, grok, qwen, or cline and ensure it is on PATH")
	}
...
var defaultAgentCommandNames = []string{
	...
	"pi", "cursor-agent", "copilot", "kimi", "kiro-cli", "codebuddy", "agy", "traecli", "grok", "qwen", "cline",
}
